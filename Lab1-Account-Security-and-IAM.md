...
**Name**: Nur Rafissah Nabila Abdul Razak
**Student ID**: 52215225324
**Course**: IKB42603 Cloud Computing Security Essentials  
**Institution**: UniKL MIIT — Nor Adani Kamal Mohamad Nasir
...
```
![Create Admin Group](Create%20admin%20group.png)  
![Attach Administrator Policy](Attach%20the%20administrator%20policy%20to%20the%20group.png)

### Step 2.2: Create Personal Admin User
```bash
aws $EP iam create-user --user-name CloudAdmin_Rafissah
```
![Create Personal Admin User](Step%204.%20create%20personal%20admin%20user.png)
...
```
![Add User to Admins Group](Step%205.%20add%20your%20user%20to%20the%20admins%20group.png)

### Step 2.4: Verify Group Membership
```bash
aws $EP iam get-group --group-name Admins
```
![Verify Group Membership](Step%206.%20verify%20the%20group%20membership.png)
...
```
![Create Analyst User](Step%207.%20create%20a%20read%20only%20analyst%20user%20(task%203).png)
...
```
![Attach Read Only Policy](Step%208.%20attach%20a%20read%20only%20policy%20to%20the%20analyst%20user.png)

### Step 3.3: Verify Attached Policies
```bash
aws $EP iam list-attached-user-policies --user-name Analyst_Rafissah
```
![Verify Attached Policy](Step%209.%20verity%20the%20attached%20policy.png)
...
```
![Create Access Key](Step%2010.%20Create%20an%20access%20key%20for%20the%20analyst.png)

### Step 4.2: List Access Keys
```bash
aws $EP iam list-access-keys --user-name Analyst_Rafissah
```
![List Access Keys](Step%2011.%20%20list%20the%20access%20keys.png)
...
```
![Deactivate Access Key](Step%2012.%20Deactive%20the%20access%20key%20(rotation%20key).png)
...
kubectl get nodes
```
![Create Kind Cluster](Session%20B.%20Step%201.%20create%20the%20kubernetes%20cluster.png)  
![Verify Kubernetes Cluster](Session%20B.%20Step%202.%20verify%20the%20kubernetes%20cluster.png)
...
```
![Create Namespaces](Session%20B.%20Step%203.%20create%20the%20namespaces.png)
...
```
![Create ServiceAccount](Session%20B.%20Step%204.%20create%20a%20service%20account.png)
...
```
![Create Pod-Reader Role](Session%20B.%20Step%205.%20create%20a%20role%20(read%20pods%20only).png)
...
```
![Create RoleBinding](Session%20B.%20Step%206.%20bind%20the%20role%20to%20the%20service%20account.png)
...
echo $SA
```
![Define SA Variable](Session%20B.%20Step%207.%20define%20the%20service%20account%20variable.png)
...
# Result: yes
```
![Test 1 YES](Session%20B.%20Step%208.%20test%201%20%20(yes).png)
...
# Result: no
```
![Test 2 NO](Session%20B.%20Step%209.%20test%202%20(no).png)
...
# Result: no
```
![Test 3 NO](Session%20B.%20Step%2010.%20Test%203%20%20(no).png)
...
- kind: ServiceAccount
  ...
  namespace: dev
```
![Last Verification](Session%20B.%20Last%20verification.png)
